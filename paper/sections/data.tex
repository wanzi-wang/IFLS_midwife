...
Treatment is assigned at the level of an individual's community of birth. Because IFLS does not record birthplace directly, I reconstruct it from migration histories. Three reconstructions are computed and preserved in the analysis frame so that downstream specifications can compare them: (i)~the legacy convention used by \textcite{cas2012} and \textcite{ahsan2021} that restricts to children whose own community identifier is unchanged across all four waves; (ii)~a variant that restricts on the mother's never-mover status rather than the child's; and (iii)~the headline variant used in the main results, which assigns the mother's community at the wave nearest preceding the child's birth. The three variants yield analytic samples of \num{5565}, \num{5638}, and \NPrimary~individuals respectively. The headline (preceding-wave) variant nearly doubles the sample relative to the legacy convention by including children whose mothers moved across waves, at the cost of a modestly noisier birthplace assignment for births before wave 1.

The headline analysis sample includes individuals born in 1984--1999 (aged 15--30 in 2014) who are alive at wave 5 and have a non-missing preceding-wave community identifier. Restricting the sample to non-migrant households would cleanly identify birthplace at the cost of dropping roughly half the cohort, which is a non-trivial source of selection in a country that experienced substantial internal migration over the relevant period, rural-to-urban migration moved from 17 to 31 percent of the population between 1971 and 1990 \autocite{alatas1993}. The headline specification therefore keeps both migrant and non-migrant individuals and treats migration as a robustness consideration rather than as a sample restriction; the alternative non-migrant and mother-never-mover samples are reported in the robustness columns of Table~\ref{tab:robustness}.
...
\paragraph{Cognition.} The cognition index combines wave-5 adult cognitive measures from Book~3B: the IRT-scored adaptive number-series score, the serial-7 subtraction count, and immediate and delayed word-recall counts. The adult IRT score covers the entire 1984--1999 cohort; the alternative child Raven score from Book~EK is administered only to respondents aged 7--14 at wave~5 and so is reported as an auxiliary outcome only in the long-paper robustness suite.
...
\begin{figure}[!htbp] \centering \includegraphics[width=0.9\textwidth]{figures/rollout_histogram.pdf} \caption{Distribution of SAR-recorded village-midwife rollout years across communities in the analysis sample.} \label{fig:rollout} \end{figure}
