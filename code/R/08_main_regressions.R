...
# Fixed effects: commid_birth_legacy + birth_year + source_wave
  ...
  m2 <- feols(
    as.formula(sprintf(
      paste0("%s ~ exposure_early_sar_legacy + sex_f + mother_edu_years + ",
             "mother_age_birth | commid_birth_legacy + birth_year + source_wave"),
      y)),
    data = data, cluster = ~ commid_birth_legacy
  )
  m3 <- feols(
...
# Control mean — substantive baseline of each outcome's primary
# anchor variable in its natural unit, computed over the combined
# untreated reference group (never-treated + pre-treatment cohorts in
# treated communities). Anchor mapping:
#   Health        -> adult height in centimetres (height_cm)
#   Cognition     -> adult IRT score (w_abil)
#   Depression    -> raw CESD-10 sum (dep_index, 0--30, lower = better)
#   Socioemotional-> Big-Five composite (bigfive_index, standardized;
#                    no natural raw unit because the five trait scores
#                    are themselves z-scored before averaging).
untreated_idx <- is.na(df$exposure_early_sar_legacy) |
                  df$exposure_early_sar_legacy == 0

anchor_spec <- list(
  health_index     = list(var = "height_cm",     fmt = "%.1f"),
  cognition_index  = list(var = "w_abil",        fmt = "%.1f"),
  depression_index = list(var = "dep_index",     fmt = "%.2f"),
  bigfive_index    = list(var = "bigfive_index", fmt = "%.3f")
)

anchor_mean_str <- function(y) {
  v  <- df[[anchor_spec[[y]]$var]]
  vv <- v[untreated_idx]; vv <- vv[!is.na(vv)]
  if (length(vv) == 0) "---" else sprintf(anchor_spec[[y]]$fmt, mean(vv))
}
...
main_coef_lines <- vapply(seq_len(nrow(main_tbl)), function(i) {
  ...
  control_cell <- anchor_mean_str(row$outcome)
  sprintf("%s & %s & %s \\\\",
...
writeLines(c(
  ...
  paste("Outcome & (1) CS & (2) dCDH &",
        "(3) TWFE & (4) TWFE + controls & Control mean \\\\"),
  "\\midrule",
  main_coef_lines,
  "\\midrule",
  ...
  sprintf("Observations & \\multicolumn{2}{c}{%s} & %s & %s & %s \\\\",
          format(main_tbl$N1[1], big.mark = ","),
          format(main_tbl$N1[1], big.mark = ","),
          format(main_tbl$N2[1], big.mark = ","),
          format(sum(untreated_idx, na.rm = TRUE), big.mark = ",")),
  "\\bottomrule",
  "\\end{tabular}"
