...
writeLines(c(
  ...
  unlist(lapply(unique(decomp$outcome), function(y) {
    ...
    lines <- apply(sub, 1, function(r) sprintf(
      "%s & %s & \\makecell{%.3f%s\\\\(%.3f)} & \\makecell{%.3f%s\\\\(%.3f)} & %s & %s \\\\",
      ifelse(r["mediator"] == sub$mediator[1], as.character(r["outcome_label"]), ""),
      as.character(r["mediator_label"]),
      as.numeric(r["beta_total"]),
      star_tex(as.numeric(r["beta_total"]), as.numeric(r["se_total"])),
      as.numeric(r["se_total"]),
      as.numeric(r["beta_cond"]),
      star_tex(as.numeric(r["beta_cond"]), as.numeric(r["se_cond"])),
      as.numeric(r["se_cond"]),
